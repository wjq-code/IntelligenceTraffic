...
public class MonitorCarFlowAnalyze {
    ...
    private static JavaPairRDD<String, String> getMonitorCarFlowTopN(JavaSparkContext jsc, long taskId, JSONObject taskParams, JavaPairRDD<Integer, String> getCarCountByMonitorInfos) {
        ...
        //遍历卡口车流量信息，把kv格式数据存储到list中：key=卡口id , value=卡口id
        for(Tuple2<Integer,String> tuple : topNCarCount){
            String monitorId = tuple._2;
            monitorId2CarCounts.add(new Tuple2<String, String>(monitorId , monitorId));
        }
        JavaPairRDD<String, String> monitorId2MonitorIdRDD = jsc.parallelizePairs(monitorId2CarCounts);
